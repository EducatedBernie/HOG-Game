...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    def swineswap(score_a,score_b):
        if max(score_a, score_b) == 2 * (min(score_a, score_b)):
    ...
    while (score < GOAL_SCORE) and (opponent_score < GOAL_SCORE):
        ...
        who = other(who)
        score,opponent_score = swineswap(score,opponent_score)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    best_number = 1
    for x in range (1,11):
        current_roll = make_averaged(roll_dice, 1000)(x, dice)
        print(x,"dice scores", current_roll, "on average")

        if max(current_roll, highest_roll) > highest_roll:
            best_number = x
            highest_roll = max(current_roll, highest_roll)
